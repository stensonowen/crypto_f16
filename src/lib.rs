...
mod tests {
    ...
    //test basic M Sequence
    #[test]
    fn prng_mseq() {
        use math::rand;
        //based on https://en.wikipedia.org/wiki/File:LFSR-F4.GIF
        // but for 16-bit and generated by hand :/
        let seed: u16 = 0b0100100010000100;
        let mseq = rand::MSequence::from(seed);
        let expected = vec![ 0b0010010001000010, 0b1001001000100001,
                0b1100100100010000, 0b0110010010001000, 0b0011001001000100,
                0b0001100100100010, 0b1000110010010001, 0b1100011001001000,
                0b0110001100100100, 0b0011000110010010, 0b1001100011001001,
                0b1100110001100100, 0b0110011000110010, 0b1011001100011001];
        let actual: Vec<u16> = mseq.into_iter().take(expected.len()).collect();
        assert_eq!(expected, actual);
    }
